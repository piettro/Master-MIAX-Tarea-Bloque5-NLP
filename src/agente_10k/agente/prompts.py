...
# v2: el bloque SALIDA dice cómo se cita. En el golden set, la mitad de los
# fallos de las extractivas eran citas parafraseadas o sin chunk_id, no
...
dónde sale el dato: "xbrl" si viene de get_xbrl_fact, "texto" si viene de un
fragmento, "ambas" si la respuesta combina las dos, "ninguna" si no está.
- `cita`: si afirmas algo tomado del texto, COPIA LITERALMENTE la frase del
  fragmento que lo dice —la que contiene la respuesta, no la primera del
  fragmento—, sin traducirla ni resumirla. Una cita parafraseada es un error.
- `chunk_id`: el identificador entre corchetes de ESE fragmento. Una cita sin
  su chunk_id no se puede verificar y cuenta como error.
- `concept_xbrl`: si la cifra sale de get_xbrl_fact, el concepto que
  consultaste, y en `cifra` el valor tal cual lo devolvió, en unidades base
  (USD, no millones).
- En una comparativa que pide cifra y explicación, fuente="ambas": la cifra de
  XBRL y la cita del texto, las dos.
"""
...
def system_prompt(version: str = VERSION_PROMPT) -> str:
    ...
    Args:
        version: Identificador de versión: "v1" o "v2", la vigente.
    ...
    Raises:
        KeyError: Si se pide una versión que no existe. Deliberado: fallar es
            ...
            traza.
    """
    versiones = {"v1": SYSTEM_V1, "v2": SYSTEM}
